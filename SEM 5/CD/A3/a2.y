...
function_parameter:
    type_specifier IDENTIFIER {local_variables.insert(string($2));int num=++func_params[func_name];temp = generate("param",num); tac.push_back(string($2) + " = " + temp); }
...
assignment_statement:
    variable '='  expression { tac.push_back(string($1) + " = " + string($3));print_tac();tac.clear();}
...
;
for_assignment_statement : variable '='  expression {tac.push_back(string($1) + " = " + string($3));}
...
/* print_parameters:
  ...
  */
expression:
    rvalue { temp= generate("t",t_number);tac.push_back(temp + " = " + string($1));strcpy($$,temp.c_str());}
...
function_call_params:
    ...
    | expression {vector<string>& vec=func_params_args.top();vec.push_back($1); }
;
